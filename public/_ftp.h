...
public:
  ...
  bool
      m_loginfailed; // 如果登录失败，用户名和密码不正确，或没有登录权限，该成员的值为true。
  ...
  bool login(const string &host, const string &username, const string &password,
  ...
  bool get(const string &remotefilename, const string &localfilename,
  ...
  bool put(const string &localfilename, const string &remotefilename,
  ...
  // 返回值：true-成功；false-失败。
  bool ftprename(const string &srcremotefilename,
