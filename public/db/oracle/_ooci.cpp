...
int oci_stmt_create(OCI_CXT* cxt, OCI_HANDLE* handle) {
  // 创建语句
  int oci_ret =
    OCIHandleAlloc(cxt->envhp, (dvoid**)&handle->smthp, OCI_HTYPE_STMT, (size_t)0, NULL);
...
int sqlstatement::execute() {
  ...
  int oci_ret =
    OCIStmtExecute(m_handle.svchp, m_handle.smthp, m_handle.errhp, m_sqltype, 0, NULL, NULL, mode);
...
int sqlstatement::alloclob() {
  ...
  return OCIDescriptorAlloc(
    (dvoid*)m_handle.envhp, (dvoid**)&m_lob, (ub4)OCI_DTYPE_LOB, (size_t)0, (dvoid**)0
  );
}
...
int sqlstatement::lobtofile(FILE* fp) {
  ...
  retval = OCILobRead(
    ...
    offset,
    (dvoid*)bufp,
    (loblen < LOBMAXBUFLEN ? loblen : LOBMAXBUFLEN),
    (dvoid*)0,
