...
int main(int argc, char *argv[]){
    ...
    if(argc!=4){
        ...
        printf("Example:/project/tools/bin/gzifiles /tmp/idc/surfdata \"*.xml,*.json\" 0.01\n");
        cout << R"(Example2:/project/tools/bin/gzifiles/log/idc "*.log.20*" 0.01)" << endl;
        printf("Example3:/project/tools/bin/procctl 300 project/tools/bin/gzipfiles /log/idc \"*.log20*\" 0.02\n\n");
        printf("Example4:/project/tools/bin/procctl 300 project/tools/bin/gzifiles /tmp/idc/surfdata \"*.xml,*json\" 0.01\n\n");

        printf("这是一个工具程序，用来压缩历史数据文件或日志文件。\n");
        printf("本程序会把pathname目录及其子目录中timeout天之前，且符合matchstr条件的文件进行压缩，timeout可以是小数。\n");
    ...
    // 将进程的信息加入到共享内存中
    pactive.addpinfo(120,"deletefiles"); // 每120秒更新一次心跳
    ...
    */
    ...
    /* 
        3-遍历目录中的文件，删除符合条件的历史数据文件
            1) 把文件的时间与历史文件的时间点进行比较，如果更早，且不是压缩文件，则进行压缩。
    */
    while(dir.readdir()==true){
        if((dir.m_mtime < strtimeout) && (matchstr(dir.m_filename,"*.gz")==false)){
            string strcmd="usr/bin/gzip -f "+dir.m_ffilename+ "1>/dev/null 2>/dev/null"; // 构造压缩命令,/dev/null(1正确，2错误时）表示不输出任何信息
